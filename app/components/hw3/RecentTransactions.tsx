...
export function RecentTransactions({ transactions }: any) {
  ...
  const renderItem = ({ item: transaction }) => <TransactionCard transactionData={transaction} lastId={transactions[transactions.length-1].id === transaction.id} />
  return (
    <View style={$RecentTransactionsContainer}>
      <View style={{ paddingBottom: 22, display:"flex", flexDirection:"row", justifyContent:"space-between", alignItems:"center"}}>
